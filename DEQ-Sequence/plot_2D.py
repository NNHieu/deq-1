...
from lib.viztool.landscape import Surface
...
def plot_2d_contour(surf_file, surf_name='train_loss', vmin=0.1, vmax=10, vlevel=0.5, show=False):
    ...
    X, Y = np.meshgrid(surface.xcoord, surface.xcoord)

    if surf_name in surface.layers.keys():
        Z = surface.layers[surf_name]
    else:
    ...
    print('len(xcoordinates): %d   len(ycoordinates): %d' % (surface.shape[0], surface.shape[1]))
